    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 cms_module_list">
            <h3 class="menuText f-s-17 hide">
                ...
                <span class="module_icon_main_text">Main Body</span>
            </h3>
            <div class=" menuBox">
                <a href="#" class="btn btn-danger addBtn"><i class="fa fa-plus"></i></a>
                <div class="selectCat">
                    {!! Form::select('type',['general'=>'General','classify'=>'Classify','email'=>'Email','customcontent'=>'Customcontent','all_posts' => 'All posts','single_post' => 'Single post'],$type,['class' => 'selectpicker select-type','data-style' => 'selectCatMenu',' data-width' => '100%']) !!}
                </div>
            </div>
            <hr>
            <ul class="list-unstyled menuList" id="components-list">
                @if(count($mainBodies))
                    @foreach($mainBodies as $layout)
                        @if($currentMainBody)
                            @if($currentMainBody->slug == $layout->slug)
                                <li class="active">
                            @else
                                <li class="">
                            @endif
                        @else
                            @if($mainBodies[0]->slug == $layout->slug)
                                <li class="active">
                            @else
                                <li class="">
                                    @endif
                                    @endif
                                    <a href="?p={!! $layout->slug !!}{!! ($type)? "&type=".$type : "" !!}" rel="unit" data-slug="{{ $layout->slug }}"
        ...
        <div class="col-xs-12 col-sm-12 col-md-9 col-lg-9">
            <div class="row">
                <div class="col-xs-12 col-sm-12 unit-box">
                    @include('uploads::gears.units._partials.main_body_box')
                </div>
            </div>
                    ...
                    <div class="template-upload-button clearfix">
                        ...
                        <ul class="editIcons list-unstyled ">
                            @if($currentMainBody && !$currentMainBody->is_core)
                                <li>
                                    <a data-href="{!! url('/admin/uploads/gears/main-body/delete') !!}" data-key="{!! $currentMainBody->slug !!}" data-type="Main Body" class="delete-button btn trashBtn"><i class="fa fa-trash-o"></i></a>
                                </li>
                            @endif
                        ...
                        <a href="{!! url('/admin/uploads/gears/main-body/settings',$currentMainBody->slug) !!}" id="viewType" class="btn btn-sm pull-right btnUploadWidgets">
            ...
            <div class="templates-list  m-t-20 m-b-10">
                <div class="row m-b-10">
                    ...
                    <div class="raw tpl-list">
                        @if($currentMainBody)
                            @include('uploads::gears.main_body._partials.variations')
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {!! Form::open(['url'=>'/admin/uploads/gears/main-body/upload','class'=>'dropzone', 'id'=>'my-awesome-dropzone']) !!}
